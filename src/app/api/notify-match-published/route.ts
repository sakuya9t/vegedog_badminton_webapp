...
export async function POST(req: NextRequest) {
  ...
  if (match.status !== 'published') return NextResponse.json({ ok: true, skipped: 'not published' })
  ...
  const winnerLine = winner === 0
    ? '本场未分出胜负。'
    : `胜方：${winner === 1 ? t1 : t2}。`
  const url     = `${process.env.NEXT_PUBLIC_SITE_URL ?? 'https://vegedog-badminton-webapp.vercel.app'}/versus`
  const subject = `${recorderName} 录入的一场对局已发布`
  const body =
`您好，

一场${match.type === 'singles' ? '单打' : '双打'}对局已全员确认并发布：
