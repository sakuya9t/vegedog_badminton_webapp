...
> 状态：**Phase 1 与 Phase 2 均已实现**。
> - Phase 1：建对局 / 录分 / 确认 / 发布 / 对战历史 / 公开性 / email 通知。
> - Phase 2：递减 K 的 ELO 积分（`apply_match_rating`，发布时按局结算）、**对战排行榜（综合 ELO 排名，二级菜单「排行榜」）** + 蛇形分组助手、全局站内信中心（`notifications` + 导航铃铛 + 未读红点 + `/notifications` + realtime）。
>
> **术语澄清**：「排行榜」是基于所有已发布对局的**综合 ELO 排名**，已上线。**「菜狗杯」是单独的 tournament（赛制 / 对阵表 / 名次），属于后续 feature，目前尚未实现**——本文档早期把排行榜归在「菜狗杯子页」是历史叫法。
...
- 改**公开性（is_public）**→ 同样作废**所有**注册参与方的确认，全员重新确认（参与方需对「是否公开」也表示同意）。
- 仅**替换某个选手**（填错换人）→ 只重置**被换的那个位**，其他人已确认保持有效。
- `published` 后**完全锁定**：不可编辑、**公开性也不可再改**；如需更正只能由录入者 `cancel` 后重建。
...
- 默认 public，创建时可切换；`draft`/`pending` 期间录入者也可改（`pending` 改动会重置全员确认）。`published` 后锁定。
- **公开性只影响可见性，不影响积分**：不公开的对局同样计入菜狗杯 ELO。创建/编辑界面会向用户说明这一点。
...
| 触发 | API 路由 | 收件人 | 内容 |
|------|----------|--------|------|
| `request_match_confirmation` 成功后（客户端 POST，**仅当录入者勾选「发送邮件通知参与方」时**；默认不勾选） | `/api/notify-match-confirmation` | 所有**非录入者**参与方 | 比分结果 + 「请在 app 内确认」+ 对战 tab 链接 |
| publish（全员确认后；由完成确认的客户端在 `confirm_match` 返回 `status='published'` 时触发） | `/api/notify-match-published` | 所有注册参与方 | 「对局已发布」+ 结果 |
...
> **底部导航现状（更新）**：五个 tab = 「接龙 / 对战 / 菜狗杯 / 赛后总结 / 设置」。
> - 「接龙」合并了原「历史」tab：`/sessions` 内用「进行中 / 历史」二级切换（`SessionsClient` + `SessionHistoryList`）；`/history` 保留为重定向到 `/sessions?tab=history`。
> - 「菜狗杯」重新作为一级 tab（`/cup`），目前是 tournament 的占位页（未实现），与「对战 → 排行榜」的综合 ELO 排名是两回事。
...
### Phase 2 ✅ 已实现
10. ✅ `player_ratings` / `rating_history` 表 + `apply_match_rating`（递减 K 的 ELO，按局结算、margin 乘子、差距阻尼、provisional、访客锚定 1000、幂等），接入 `confirm_match` 的 publish 分支。
11. ✅ 「排行榜」子页（[Leaderboard.tsx](../src/app/(tabs)/versus/Leaderboard.tsx)）：**综合对战排行榜**（名次/积分/局数/最近变动 + realtime）+ 蛇形分组助手；球员主页显示排位分数与名次。（菜狗杯 tournament 仍是后续 feature。）
12. ✅ 全局站内信中心：`notifications` 表 + 导航铃铛（[NotificationBell.tsx](../src/components/NotificationBell.tsx)）+ 未读红点 + [/notifications](../src/app/notifications/page.tsx) + realtime。确认请求 / 发布在 `request_match_confirmation`、`confirm_match` 内写入；关注新接龙 / 候补递补在对应 notify API 路由写入。

> **站内信 vs 邮件**：站内信对事件相关方**始终**生成（与邮件 opt-out 偏好无关，GitHub 风格）；设置页的「邮件通知」开关只控制邮件渠道。
...
- **单一来源**：`supabase/schema.sql` 的「8 / 8b / 8c」整块是权威定义（给全新项目）。
- **托管增量**（按顺序跑，dev 先、prod 后）：
  1. `supabase/migrations_versus.sql` — Phase 1 对战表 / RPC / RLS（仅首次）。
  2. `supabase/migrations_versus_phase2.sql` — Phase 2 积分：`player_ratings` / `rating_history` + `apply_match_rating` + 更新 `confirm_match`。
  3. `supabase/migrations_notifications.sql` — 站内信：`notifications` 表 + 更新 `request_match_confirmation` / `confirm_match`（依赖第 2 步的 `apply_match_rating`）。
  4. `supabase/migrations_notify_prefs.sql` — `profiles` 邮件通知开关列（与对战无直接关系，但同批发布）。
  > 改 schema.sql 后需同步维护对应迁移文件。表 / policy / realtime 为 run-once，函数为 `create or replace`（可重跑）。
